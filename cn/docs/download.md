# 下载

## 桌面 App


- [<i class="bi bi-apple"></i> macOS (Intel)](https://paperlib.app/distribution/electron-mac/latest.dmg)
- [<i class="bi bi-apple"></i> macOS (silicon)](https://paperlib.app/distribution/electron-mac-arm/latest.dmg)
- [<i class="bi bi-microsoft"></i> Windows](https://paperlib.app/distribution/electron-win/latest.zip)
- [<i class="bi bi-ubuntu"></i> Linux](/download-linux)

## 浏览器扩展

- [<i class="bi bi-browser-chrome"></i> Chrome / Edge](https://chrome.google.com/webstore/detail/paperlib/kgnpnfbjckgddlednhoblgfdfohhapng)
- [<i class="bi bi-browser-firefox"></i> Firefox](https://addons.mozilla.org/firefox/addon/paperlib/)
- [<i class="bi bi-browser-safari"></i> Safari](/doc/extensions/browser-extension.html#safari-extension)

## MS Word 扩展

- [<i class="bi bi-file-earmark-word-fill"></i> MS Word](/doc/extensions/msword-extension)

